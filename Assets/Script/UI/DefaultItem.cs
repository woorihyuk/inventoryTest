...
namespace Script.UI
{
    public class DefaultItem : MonoBehaviour, IBeginDragHandler, IDragHandler, IEndDragHandler
    {
        ...
        public void OnBeginDrag(PointerEventData eventData)
        {
            ...
            if (_isInInventory)
            {
                for (var i = 0; i < countY; i++)
                {
                    ...
                    {
                        _inGameUiManager.InventoryGrid[j + _gridPosX, i + _gridPosY].isOverlap = false;
                    } 
                }
            }
            else
        ...
        public void OnEndDrag(PointerEventData eventData)
        {
            ...
            {
                OverLapped(_lastGrids, _gridPosX, _gridPosY);
        ...
        private void OverLapped(ItemGrid[,] itemGrids, int posX, int posY)
        {
            itemGrids[posX, posY].isOverlap = true;
            for (var i = 0; i < countY; i++)
            {
                for (var j = 0; j < countX; j++)
                {
                    itemGrids[j + posX, i + posY].isOverlap = true;
                }
            }
        }
        ...
        protected virtual void InStack()
        {
